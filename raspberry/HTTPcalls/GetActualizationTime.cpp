...
bool GetActualizationTime::success = false;

size_t
GetActualizationTime::handle_impl(void *buffer, size_t size, size_t nmemb)
{
    struct json_object *place;
    struct json_object *actualizationTime;
    struct json_object *parsed_json;
    parsed_json = json_tokener_parse((char *)buffer);
    if (json_object_object_get_ex(parsed_json, "place", &place))
    {
        if (json_object_object_get_ex(place, "actualizationTime", &actualizationTime))
        {
...
bool GetActualizationTime::call()
{
    ...
    GetActualizationTime::success = false;
    if (!this->curl)
        return false;
    char aux[2000];
    char auxURL[2000];
